using System.Runtime.Serialization;

namespace GoogleAPI.Maps.Model.Geocoding {
    [DataContract]
    public class AddressComponent {
        /// <summary>
        /// Is the full text description or name of the address component as returned by the Geocoder.
        /// </summary>
        [DataMember(Name="long_name")]
        public string LongName { get; set; }
        /// <summary>
        /// Is an abbreviated textual name for the address component, if available. For example, 
        /// an address component for the state of Alaska may have a long_name of "Alaska" and a 
        /// short_name of "AK" using the 2-letter postal abbreviation.
        /// </summary>
        [DataMember(Name = "short_name")]
        public string ShortName { get; set; }
        /// <summary>
        /// Is an array indicating the type of the address component.
        /// </summary>
        [DataMember(Name = "types")]
        public string[] GeoTypes { get; set; }
    }
}
